...
describe("당첨 로또 번호 입력와 보너스 번호 입력에 공통적으로 적용되는 검증", () => {
  ...
  test("숫자는 비어있을 수 없습니다.", () => {
    ...
    expect(() => result.setWinningNumbers(number)).toThrow("[Error]");
  });
...
describe("당첨 로또 번호 입력 시 적용되는 검증", () => {
  ...
  test("배열 안의 값은 6개여야 합니다.", () => {
    ...
    const result = new LottoResult();
    expect(() => result.setWinningNumbers(numbers)).toThrow("[Error]");
  });
...
describe("보너스 번호 입력 시 적용되는 검증", () => {
  ...
  test("보너스 번호는 당첨 로또 번호와 중복될 수 없습니다.", () => {
    ...
    const bonusNumber = "1";
    expect(() => result.setBonusNumber(bonusNumber)).toThrow("[Error]");
  });
});
